...
const anyone_around = require('../assets/images/anyone_around@3x.png');
const pso_finder = require('../assets/images/pso_finder@3x.png');
  ...
  if (item.key == 's1') {
    return (
      <>
        <View
          ...
          <Text style={slider_styles.introTitleStyle}>{item.title}</Text>
          <Image
            style={[slider_styles.MainImageStyle, { backgroundColor: null }]}
            source={item.image}
          />
          <Text style={[slider_styles.introTextStyle, { marginBottom: 0 }]}>
            {item.text}
          </Text>
        </View>
        <View
          ...
          <Image
            style={[slider_styles.SubImageStyle, { tintColor: '#2d327a' }]}
            source={item.image1}
          />
          <Image style={slider_styles.SubImageStyle} source={item.image2} />
          <Image style={slider_styles.SubImageStyle} source={item.image3} />
        </View>
        <View
          style={{
            flex: 1,
          ...
          <Text style={slider_styles.subtextStyle}>{item.subtext1}</Text>
          <Text style={slider_styles.subtextStyle}>{item.subtext2}</Text>
          <Text style={slider_styles.subtextStyle}>{item.subtext3}</Text>
        </View>
      </>
    );
  } else {
    return (
      <View
        ...
        <Text style={slider_styles.introTitleStyle}> {item.title} </Text>
        {item.key != 's2' ? (
          <Image style={slider_styles.MainImageStyle} source={item.image} />
        ) : (
          <Image
            style={[slider_styles.MainImageStyle, { tintColor: COLORS.dark }]}
            source={item.image}
          />
        )}

        <Text style={slider_styles.introTextStyle}> {item.text} </Text>
      </View>
    );
  }
};
...
const slider_styles = StyleSheet.create({
  subtextStyle: {
    padding: 20,
    textAlign: 'center',
    fontSize: 18,
